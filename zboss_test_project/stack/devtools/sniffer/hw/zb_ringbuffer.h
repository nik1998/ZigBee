 ...
 * @param rb - ring buffer pointer.
 */
#define ZB_RING_BUFFER_IS_FULL(rb) ((zb_uint16_t)(rb)->written >= ZB_RING_BUFFER_CAPACITY(rb))
 ...
 */
#define ZB_RING_BUFFER_READ_PEEK(rb)                 \
(                                               \
  ZB_RING_BUFFER_IS_EMPTY(rb) ? NULL            \
  : (rb)->ring_buf + (rb)->read_i               \
  )

#define ZB_RING_BUFFER_WRITE_PEEK(rb) ((rb)->ring_buf + (rb)->write_i)
#define ZB_RING_BUFFER_WRITTEN(rb) ((rb)->written++, (rb)->write_i = ((rb)->write_i + 1) % ZB_RING_BUFFER_CAPACITY(rb))
#define ZB_RING_BUFFER_READ(rb) ((rb)->written--, (rb)->read_i = ((rb)->read_i + 1) % ZB_RING_BUFFER_CAPACITY(rb))
